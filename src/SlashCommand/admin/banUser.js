...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        try {
            ...
            if(target.bot){
                console.error('Я не могу выгнать сам забанить.')
                return interaction.reply({content: `❌ Я не могу забанить себя!`,ephemeral: true})
            }
            if(member.permissions.has(PermissionsBitField.Flags.Administrator)){
                console.error('У пользователя статус Администратора.')
                return interaction.reply({content: `❌ Я не могу забанить данного пользователя!`,ephemeral: true})
            }

            const embed_info = new EmbedBuilder()
                .setColor('Red')
                //.setTitle(`Выгнали ${target.username}`)
                .setAuthor({ name: `Банит ${target.username}`, iconURL: interaction.user.displayAvatarURL({ dynamic: true })})
